...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (userStats?.level && prevLevel.current !== null && userStats.level > prevLevel.current) {
      toast.success(`Level Up: LVL ${userStats.level}`);
    }
    prevLevel.current = userStats?.level || null;
  }, [userStats]);

  const progressPercentage = (userStats.xp / userStats.requiredXp) * 100;
      ...
      <nav className="sticky top-0 z-50 bg-white/60 backdrop-blur-2xl border-b border-slate-100/50">
        <div className="max-w-7xl mx-auto px-8 h-24 flex justify-between items-center">
          ...
          <div className="bg-slate-900 text-white px-6 py-2.5 rounded-2xl flex items-center gap-3 shadow-xl shadow-slate-200">
            <span className="text-xs font-black tracking-widest uppercase text-slate-300">Level {userStats.level}</span>
        ...
        <div className="lg:col-span-4 space-y-12">
          <section className="bg-white rounded-[3.5rem] p-10 shadow-sm border border-slate-100/50">
            ...
            <div className="space-y-4">
              {[
                { id: 'session', label: 'Start First Session', checked: userStats.xp > 0, auto: true },
                { id: 'studyTwoHours', label: '2h Focus Target', checked: false },
                { id: 'feedMing', label: 'Clean Mascot Habitat', checked: false },
              ].map((goal) => (
